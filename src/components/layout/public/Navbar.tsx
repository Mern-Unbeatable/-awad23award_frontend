...
export function Navbar() {
  ...
  const isContact =
    current.startsWith('/contact') || current.startsWith('/book');
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        const lenis = (window as Window & { __lenis?: { scroll: number } })
          .__lenis;
  ...
  const brandLabel =
    settings.brandName && !/official/i.test(settings.brandName)
      ? settings.brandName
      : 'Ahmed Ibrahim';
    ...
    >
      <div className='w-full max-w-360 mx-auto px-6 sm:px-10 lg:px-16 flex items-center justify-between h-18.5 gap-4'>
        ...
        <nav className='hidden lg:flex items-center gap-6'>
          ...
          <Link
            to={pathFor('/contact')}
            className={navClass(isContact && !current.startsWith('/book'))}
          >
            {t('Contact', 'تواصل')}
          </Link>
      ...
      {open && (
        <div
          ref={menuRef}
          className='fixed inset-0 z-50 bg-[#064738] text-white flex flex-col justify-between p-6 sm:px-10 overflow-y-auto animate-in fade-in duration-200 lg:hidden'
        >
          ...
          <nav className='flex flex-col space-y-2 my-auto py-6'>
            <Link
              to={pathFor('/')}
              onClick={() => setOpen(false)}
              className='text-xl font-semibold text-white hover:text-[#35BFFB] py-2 border-b border-white/10'
            >
              {t('Home', 'الرئيسية')}
            </Link>
            <Link
